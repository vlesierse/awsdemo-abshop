...
export class ClusterAutoScaler extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, props: ClusterAutoScalerProps) {
    ...
    const { cluster, autoScalingGroup } = props;
    const version = props.version ?? 'v1.17.3';
    ...
    autoscalerPolicy.attachToRole(autoScalingGroup.role);

    cdk.Tags.of(autoScalingGroup).add(`k8s.io/cluster-autoscaler/${cluster.clusterName}`, "owned", { applyToLaunchedInstances: true });
    cdk.Tags.of(autoScalingGroup).add("k8s.io/cluster-autoscaler/enabled", "true", { applyToLaunchedInstances: true });
